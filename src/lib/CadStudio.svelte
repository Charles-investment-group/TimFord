<script>
	...
	// Faceted pyramid: triangles fanned around the Y axis, each leaning in so the
	// apexes converge at the peak. Shade is baked per facet so the mesh reads as
		...
		<div class="layout">
			<div class="viewport">
				...
				<div
					class="stage"
					role="img"
					aria-label="Preview of a rotating low-poly 3D model on a perspective grid"
				>
					<div class="floor"></div>
						...
						<div class="tilt">
							<div class="mesh">
								{#each facets as facet}
									<span
										class="facet"
										style="--a: {facet.angle}deg; --b: {facet.brightness};"
									></span>
								{/each}
							</div>
						</div>
...
<style>
	...
	.intro {
		text-align: center;
		/* wide enough for the heading to hold one line at its cap size */
		max-width: 780px;
	...
	.eyebrow {
		...
		color: #58aaff;
		font-size: clamp(0.6rem, 1.4vw, 0.66rem);
		font-weight: 700;
	...
	h2 {
		margin: 0 0 1.1rem;
		/* the vw term is tuned so 34 characters always fit the container width */
		font-size: clamp(0.8rem, 4.4vw, 2.5rem);
		line-height: 1.15;
	...
	.stage {
		...
		font-size: clamp(11px, 2.7vw, 19px);
		perspective: 34em;
	...
	.cta:focus-visible {
		background: #2f83ef;
		box-shadow: 0 14px 34px -18px rgba(30, 111, 217, 0.95);
	}

	/* ---------- responsive ---------- */

	/* stack the viewport above the panel */
